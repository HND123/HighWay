...
CREATE TABLE IF NOT EXISTS `highway`.`passenger` (
  ...
  `Passengercol` VARCHAR(45) NULL DEFAULT NULL,
  `bus_Bus_No` INT(11) NOT NULL,
  PRIMARY KEY (`User_ID`),
  INDEX `fk_passenger_bus1_idx` (`bus_Bus_No` ASC),
  CONSTRAINT `fk_passenger_bus1`
    FOREIGN KEY (`bus_Bus_No`)
    REFERENCES `highway`.`bus` (`Bus_No`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `highway`.`bus` (
  ...
  `bus_sheets_Sheet_No` INT(11) NOT NULL,
  `schedule_Bus_No` INT(11) NOT NULL,
  PRIMARY KEY (`Bus_No`),
  INDEX `fk_bus_passenger_idx` (`passenger_User_ID` ASC),
  INDEX `fk_bus_bus_sheets1_idx` (`bus_sheets_Sheet_No` ASC),
  ...
  CONSTRAINT `fk_bus_passenger`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_bus_bus_sheets1`
    FOREIGN KEY (`bus_sheets_Sheet_No`)
    REFERENCES `highway`.`bus_sheets` (`Sheet_No`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `highway`.`other_vehicles` (
  ...
  `Ticket_price` VARCHAR(45) NULL DEFAULT NULL,
  `Vihicle_type` VARCHAR(45) NULL DEFAULT NULL,
